...
def calculate_max_acid_dose_by_nutrient_limit(
    ...
    logger.info(
        f"[AcidExpert] {acid_id}: deficit {nutrient_key}={deficit:.1f}, max dose={max_dose:.1f} mL/1000L (110% = {max_contribution_kg:.1f} kg/ha)"
    )
...
def recommend_acids_for_fertiirrigation(
    ...
    Principles:
    1. No nutrient from acids can exceed 110% of its deficit
        ...
        if not candidates:
            if remaining_neutralization > 0.5:
                warnings.append(
                    ...
                    f"Los límites de cobertura de nutrientes (110%) impiden añadir más ácidos."
                )
            break
    ...
    for nutrient in ['N', 'P', 'S']:
        ...
        if orig > 0:
            ...
            logger.info(
                f"[AcidExpert] {nutrient} coverage from acids: {coverage_pct:.1f}% (max 110%)"
            )
...
def get_acid_coverage_constraints(
    ...
    With the new acid system limiting contributions to 110%, this function:
    ...
    4. Always coordinate acid+fertilizer selection to not exceed 110% total
    ...
    if acid_s > 0:
        if s_deficit > 0:
            ...
            remaining_s = max(0, s_deficit * 1.10 - acid_s)
        else:
            s_coverage_pct = 110.0
            remaining_s = 0
    ...
    if acid_n > 0:
        if n_deficit > 0:
            ...
            remaining_n = max(0, n_deficit * 1.10 - acid_n)
        else:
            n_coverage_pct = 110.0
            remaining_n = 0
    ...
    if acid_p > 0:
        ...
        if p_deficit > 0:
            ...
            remaining_p = max(0, p_deficit * 1.10 - p2o5_from_acid)
        else:
            p_coverage_pct = 110.0
            remaining_p = 0
...
def apply_acid_constraints_to_fertilizers(
    ...
    This ensures total coverage (acid + fertilizers) doesn't exceed 110%.
...
MAX_COVERAGE_RATIO = 1.10  # 110% maximum (aligned with acid limit)
...
def _load_hydro_fertilizers_catalog() -> List[Dict]:
    ...
    for fert in fertilizers:
        ...
        nh4_n_pct = nutrient_comp.get("NH4_N_percent", 0) or 0
        ...
        k2o_pct = nutrient_comp.get("K2O_percent", 0) or 0
        if k2o_pct == 0:
            k_pct = nutrient_comp.get("K_percent", 0) or 0
            k2o_pct = k_pct * 1.205 if k_pct > 0 else 0

        p2o5_pct = nutrient_comp.get("P2O5_percent", 0) or 0
        if p2o5_pct == 0:
...
def get_available_fertilizers_for_user(db,
    ...
    for fert_id in visible_ids:
        ...
        # Initialize all nutrient percentages to avoid stale values
        n_pct = 0
        nh4_n_pct = 0
        p2o5_pct = 0
        k2o_pct = 0
        ...
        if hydro_fert:
            ...
            nh4_n_pct = hydro_fert.get('NH4_N_percent', 0) or hydro_fert.get('nh4_n_pct', 0) or 0
            ...
            try:
                ...
                if db_product:
                    ...
                    nh4_n_pct = 0  # DB products don't track NH4 separately
        ...
        result.append({
            ...
            'nh4_n_pct': round(nh4_n_pct, 2) if nh4_n_pct else 0,
            'NH4_N_percent': round(nh4_n_pct, 2) if nh4_n_pct else 0,
...
def _score_fertilizer_v2(
    ...
    dose, coverage_deltas = _calculate_constrained_dose(
        ...
        max_coverage_pct, priority_nutrients, allow_carrier_overshoot=False)
    
    if dose < 0.1 and priority_nutrients:
        dose, coverage_deltas = _calculate_constrained_dose(
            fert, remaining_deficits, original_deficits, current_contributions,
            max_coverage_pct, priority_nutrients, allow_carrier_overshoot=True)
...
CONTROLLED_OVERSHOOT_LIMIT = 1.13
...
def _calculate_constrained_dose(fert: Dict,
                                ...
                                allow_carrier_overshoot: bool = False) -> tuple:
    """
    Calculate the maximum dose that respects the 110% cap on ALL nutrients.

    Returns: (constrained_dose, coverage_deltas)
    - constrained_dose: The max kg/ha we can apply without exceeding 110% on ANY nutrient
    - coverage_deltas: Dict of coverage % gain for each nutrient at this dose

    STRICT LIMIT: The dose is limited by whichever nutrient first reaches 110%.
    If any nutrient has no headroom (already at 110%), dose for that nutrient = 0.
    Also enforces 110% cap on NH4 separately (not in NUTRIENT_PRIORITY).
    
    CONTROLLED OVERSHOOT: When allow_carrier_overshoot=True, secondary nutrients
    (Ca, Mg, S, K2O when not priority) can overshoot to 113% to enable coverage
    of blocked priority nutrients.
    """
    max_allowed_doses = []
    can_contribute_to_priority = False
    carrier_nutrients = {'Ca', 'Mg', 'S', 'K2O'}
    ...
    for nutrient in NUTRIENT_PRIORITY:
        ...
        effective_max_pct = max_coverage_pct
        if allow_carrier_overshoot and nutrient in carrier_nutrients and nutrient not in priority_nutrients:
            effective_max_pct = CONTROLLED_OVERSHOOT_LIMIT
        
        max_total = original * effective_max_pct
    ...
    nh4_content = _get_nutrient_content(fert, 'NH4')
    nh4_deficit = original_deficits.get('NH4', 0)
    if nh4_content > 0 and nh4_deficit > 0:
        nh4_covered = current_contributions.get('NH4', 0)
        
        nh4_effective_max = max_coverage_pct
        if allow_carrier_overshoot and 'NH4' not in priority_nutrients:
            nh4_effective_max = CONTROLLED_OVERSHOOT_LIMIT
        
        nh4_max_total = nh4_deficit * nh4_effective_max
        nh4_headroom = max(0, nh4_max_total - nh4_covered)
        if nh4_headroom > 0:
            nh4_max_dose = nh4_headroom / (nh4_content / 100)
            max_allowed_doses.append(nh4_max_dose)
        else:
            return 0, {}
...
def _apply_dose(fert: Dict, dose_kg: float, remaining_deficits: Dict[str,
    ...
    nh4_content = _get_nutrient_content(fert, 'NH4')
    if nh4_content > 0:
        nh4_contribution = dose_kg * (nh4_content / 100)
        contributions['NH4'] = nh4_contribution
        remaining_deficits['NH4'] = max(
            0,
            remaining_deficits.get('NH4', 0) - nh4_contribution)
...
def _optimize_profile(
        ...
        original_deficits: Optional[Dict[str, float]] = None,
        acid_contributions: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """
    ...
    COVERAGE LIMITS (95%-110%):
    - Initialize current_contributions with acid contributions to respect total limits
    - Use original_deficits for coverage calculations to ensure accurate % limits
    """
    config = PROFILE_CONFIGS[profile_key]
    
    real_deficits = original_deficits if original_deficits else deficits
    ...
    current_contributions['NH4'] = 0.0
    
    if acid_contributions:
        current_contributions['N'] = acid_contributions.get('N', 0)
        current_contributions['P2O5'] = acid_contributions.get('P', 0) * 2.29
        current_contributions['S'] = acid_contributions.get('S', 0)
        logger.info(f"[_optimize_profile] Initialized with acid contributions: N={current_contributions['N']:.2f}, P2O5={current_contributions['P2O5']:.2f}, S={current_contributions['S']:.2f}")
    ...
    for sec_nutrient in secondary_nutrients[:reserved_slots]:
        ...
        current_sec_cov = current_contributions.get(
            sec_nutrient, 0) / real_deficits[sec_nutrient] * 100 if real_deficits.get(
        ...
        for fert in available:
            ...
            dose, deltas = _calculate_constrained_dose(
                fert, remaining_deficits, real_deficits, current_contributions,
    ...
    for scarce_nutrient in scarce_nutrients:
        current_coverage = _calculate_coverage(real_deficits, remaining_deficits)
        ...
        for n in NUTRIENT_PRIORITY:
            ...
            if cov > MAX_COVERAGE_LIMIT * 100 and deficits.get(n, 0) > 0:
                liebig_overrides.append({
                    ...
                    round(cov, 1),
                    'reason':
                    f'{n} excede 110% para permitir cobertura de {target_nutrient} (Ley de Liebig)'
                })
        rescue_nutrients = [
            n for n in NUTRIENT_PRIORITY if deficits.get(n, 0) > 0
            and current_coverage.get(n, 0) < liebig_threshold
    ...
    final_coverage = _calculate_coverage(real_deficits, remaining_deficits)
...
def optimize_deterministic(
    ...
    for f in available_fertilizers:
        ...
        comp = {
            ...
            'nh4_n_pct': f.get('nh4_n_pct') or f.get('NH4_N_percent') or f.get('analysis_nh4_n_pct', 0) or 0,
    ...
    def normalize_profile(profile,
        ...
        for fert in profile.get('fertilizers', []):
            ...
            has_valid_existing = existing_contribs and any(v > 0 for v in existing_contribs.values())
            has_composition_data = 'n_pct' in comp
            
            logger.info(f"[NormalizeProfile] Fert: {fert.get('name')}, dose={dose}, existing_contribs={existing_contribs}, has_valid={has_valid_existing}, has_comp={has_composition_data}")
            
            if has_valid_existing:
                logger.info(f"[NormalizeProfile] PRESERVING existing contributions for {fert.get('name')}")
                # Add NH4 contribution if fertilizer has nh4_n_pct but contributions don't include it
                nh4_n_pct = comp.get('nh4_n_pct', 0)
                if nh4_n_pct > 0 and 'NH4' not in existing_contribs:
                    nh4_contrib = dose * (nh4_n_pct / 100)
                    fert['contributions'] = {**existing_contribs, 'NH4': round(nh4_contrib, 2)}
                    logger.info(f"[NormalizeProfile] Added NH4={nh4_contrib:.2f} to {fert.get('name')}")
            elif has_composition_data:
                calculated_contribs = {
                    'N': dose * (comp.get('n_pct', 0) / 100),
                    'NH4': dose * (comp.get('nh4_n_pct', 0) / 100),
            ...
            if has_composition_data:
                fert['n_pct'] = comp.get('n_pct', 0)
                fert['nh4_n_pct'] = comp.get('nh4_n_pct', 0)
                fert['p2o5_pct'] = comp.get('p2o5_pct', 0)
                fert['k2o_pct'] = comp.get('k2o_pct', 0)
                fert['ca_pct'] = comp.get('ca_pct', 0)
                fert['mg_pct'] = comp.get('mg_pct', 0)
                fert['s_pct'] = comp.get('s_pct', 0)

        if acid_contribs:
            logger.info(f"[NormalizeProfile] Acid contributions added: N={acid_contribs.get('N', 0)}, P={acid_contribs.get('P', 0)} (P2O5={acid_contribs.get('P', 0) * 2.29:.2f}), S={acid_contribs.get('S', 0)}")
            logger.info(f"[NormalizeProfile] Total contributions with acids: {contributions}")

        nutrients_to_check = list(NUTRIENT_PRIORITY) + ['NH4']
        
        def get_nutrient_pct(comp, nutrient):
            if 'n_pct' not in comp:
                return 0
            mapping = {
                'N': 'n_pct', 'P2O5': 'p2o5_pct', 'K2O': 'k2o_pct',
                'Ca': 'ca_pct', 'Mg': 'mg_pct', 'S': 's_pct', 'NH4': 'nh4_n_pct'
            }
            return comp.get(mapping.get(nutrient, ''), 0)
        
        def recalculate_contributions():
            recalc = {n: 0.0 for n in nutrients_to_check}
            if acid_contribs:
                recalc['N'] += acid_contribs.get('N', 0)
                recalc['P2O5'] += acid_contribs.get('P', 0) * 2.29
                recalc['S'] += acid_contribs.get('S', 0)
            for fert in profile.get('fertilizers', []):
                dose = fert.get('dose_kg_ha', 0) or 0
                comp = resolve_fert_composition(fert)
                for n in nutrients_to_check:
                    pct = get_nutrient_pct(comp, n)
                    if pct > 0:
                        recalc[n] += dose * (pct / 100)
            return recalc
        
        def check_bounds():
            """Check if all nutrients are within 95%-110% bounds. Returns (all_ok, issues)."""
            issues = {'over': [], 'under': []}
            contribs = recalculate_contributions()
            for nutrient in nutrients_to_check:
                deficit = deficits_dict.get(nutrient, 0)
                if deficit > 0:
                    covered = contribs.get(nutrient, 0)
                    cov_pct = (covered / deficit) * 100
                    if cov_pct > 110:
                        issues['over'].append((nutrient, cov_pct, covered - deficit * 1.10))
                    elif cov_pct < 95:
                        issues['under'].append((nutrient, cov_pct, deficit * 0.95 - covered))
            all_ok = len(issues['over']) == 0 and len(issues['under']) == 0
            return all_ok, issues, contribs
        
        def clamp_excess(issues, contribs):
            """Reduce doses to eliminate nutrients over 110%."""
            for nutrient, cov_pct, excess in issues['over']:
                for fert in profile.get('fertilizers', []):
                    if excess <= 0.01:
                        break
                    comp = resolve_fert_composition(fert)
                    content_pct = get_nutrient_pct(comp, nutrient)
                    if content_pct > 0:
                        dose = fert.get('dose_kg_ha', 0) or 0
                        if dose <= 0:
                            continue
                        fert_contribution = dose * (content_pct / 100)
                        reduction = min(fert_contribution, excess)
                        dose_reduction = reduction / (content_pct / 100)
                        new_dose = max(0, dose - dose_reduction)
                        fert['dose_kg_ha'] = round(new_dose, 2)
                        fert['dose_per_application'] = round(new_dose / max(1, n_apps), 2)
                        excess -= reduction
                        if 'contributions' in fert and dose > 0:
                            scale = new_dose / dose
                            for k in fert['contributions']:
                                if fert['contributions'][k] > 0:
                                    fert['contributions'][k] = round(fert['contributions'][k] * scale, 2)
        
        def refill_deficit(issues, contribs):
            """Increase doses to restore nutrients under 95%."""
            made_change = False
            for nutrient, cov_pct, need in issues['under']:
                for fert in profile.get('fertilizers', []):
                    if need <= 0.5:
                        break
                    comp = resolve_fert_composition(fert)
                    ...
                    current_dose = fert.get('dose_kg_ha', 0) or 0
                    dose_increase_needed = need / (content_pct / 100)
                    
                    fresh_contribs = recalculate_contributions()
                    max_increase = dose_increase_needed
                    can_increase = True
                    
                    for n2 in nutrients_to_check:
                        n2_pct = get_nutrient_pct(comp, n2)
                        if n2_pct > 0:
                            d2 = deficits_dict.get(n2, 0)
                            if d2 > 0:
                                c2 = fresh_contribs.get(n2, 0)
                                headroom = d2 * MAX_COVERAGE_RATIO - c2
                                if headroom <= 0:
                                    can_increase = False
                                    break
                                max_dose_for_n2 = headroom / (n2_pct / 100)
                                max_increase = min(max_increase, max_dose_for_n2)
                    
                    if can_increase and max_increase >= 0.1:
                        actual_increase = min(dose_increase_needed, max_increase)
                        new_dose = current_dose + actual_increase
                        fert['dose_kg_ha'] = round(new_dose, 2)
                        fert['dose_per_application'] = round(new_dose / max(1, n_apps), 2)
                        need -= actual_increase * (content_pct / 100)
                        made_change = True
                        if 'contributions' in fert and current_dose > 0:
                            scale = new_dose / current_dose
                            for k in fert['contributions']:
                                if fert['contributions'][k] > 0:
                                    fert['contributions'][k] = round(fert['contributions'][k] * scale, 2)
            return made_change
        ...
        max_iterations = 15
        for iteration in range(max_iterations):
            all_ok, issues, contributions = check_bounds()
            if all_ok:
                logger.info(f"[NormalizeProfile] All nutrients within 95%-110% after {iteration} iterations")
                break
            
            if issues['over']:
                clamp_excess(issues, contributions)
            ...
            if issues['under']:
                made_change = refill_deficit(issues, contributions)
                if not made_change:
                    logger.warning(f"[NormalizeProfile] Cannot refill deficient nutrients - infeasible scenario")
                    break
        
        contributions = recalculate_contributions()
        ...
        for nutrient in nutrients_to_check:
            ...
            if deficit > 0:
                ...
                cov_pct = (covered / deficit) * 100 if deficit > 0 else 0
                ...
                if cov_pct > 110:
                    coverage_warnings.append({
                        ...
                        'message': f"{nutrient} limitado a 110% (era {cov_pct:.1f}%)"
                    })
                elif cov_pct < 95:
                    blocked_by = []
                    for n2 in nutrients_to_check:
                        if n2 != nutrient:
                            d2 = deficits_dict.get(n2, 0)
                            c2 = contributions.get(n2, 0)
                            if d2 > 0 and (c2 / d2) >= 1.08:
                                blocked_by.append(n2)
                    
                    reason = "conflictos con otros nutrientes saturados" if blocked_by else "limitaciones del catálogo de fertilizantes"
                    infeasibility_notes.append({
                        'nutrient': nutrient,
                        'type': 'infeasible',
                        'coverage': round(cov_pct, 1),
                        'deficit_kg_ha': round(deficit * (0.95 - cov_pct/100), 2),
                        'blocked_by': blocked_by,
                        'message': f"{nutrient} en {cov_pct:.1f}% - {reason}"
                    })
        
        if infeasibility_notes or coverage_warnings:
            profile['coverage_diagnostics'] = {
                'infeasible': infeasibility_notes,
                'capped': coverage_warnings
            }
            if infeasibility_notes:
                logger.warning(f"[NormalizeProfile] Infeasible nutrients: {[n['nutrient'] for n in infeasibility_notes]}")
        ...
        for nutrient in NUTRIENT_PRIORITY:
            ...
            if deficit > 0:
                covered = contributions.get(nutrient, 0)
                cov_pct = min(round((covered / deficit) * 100, 1), 110.0)
        ...
        nh4_deficit = deficits_dict.get('NH4', 0)
        if nh4_deficit > 0:
            nh4_covered = contributions.get('NH4', 0)
            nh4_cov_pct = min(round((nh4_covered / nh4_deficit) * 100, 1), 110.0) if nh4_deficit > 0 else 0
            coverage['NH4'] = nh4_cov_pct
    ...
    for profile_key in ['economic', 'balanced', 'complete']:
        ...
        profile = _optimize_profile(adjusted_deficits, micro_deficits,
                                    ...
                                    original_deficits=deficits,
                                    acid_contributions=acid_contribs)
