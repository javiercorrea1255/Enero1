...
export default function FertiIrrigationCalculator() {
  ...
  const transformAIResponse = (aiRes, acidRec = null) => {
    ...
    return { 
      ...
      currency: userCurrency.code,
      deficits: aiRes.deficits || null,
      adjusted_deficits: aiRes.adjusted_deficits || null
    };
  };
  ...
  const renderStep6 = () => {
    ...
    const acidsForTable = (acidProgram?.acids || []).map(acid => {
      ...
      return {
        ...
        contributions: {
          N: contribs.N || 0,
          P2O5: (contribs.P || 0) * 2.29, // Convert P to P2O5
          S: contribs.S || 0
        },
        n_pct: 0,
        p2o5_pct: 0,
    ...
    const getContributionValue = (fertilizer, nutrientKey, pctField, dose) => {
      ...
      const pct = nutrientKey === 'NH4'
        ? fertilizer.nh4_n_pct || fertilizer[pctField] || fertilizer.nutrient_composition?.NH4_N_percent || fertilizer.nutrient_composition?.NH4_percent || 0
        : fertilizer[pctField] || fertilizer.nutrient_composition?.[`${nutrientKey}_percent`] || 0;
      return dose * pct / 100;
    };

    // Combined source: fertilizers + acids for table display
    const fertAndAcidsSource = [...fertSource, ...acidsForTable];
    
    // Calculate totals including acids
    const acidContribTotals = acidsForTable.reduce((acc, acid) => {
      acc.N += acid.contributions?.N || 0;
      acc.P2O5 += acid.contributions?.P2O5 || 0;
      acc.S += acid.contributions?.S || 0;
      return acc;
    }, { N: 0, P2O5: 0, S: 0 });
    
    const contributionTotals = {
      N: fertSource.reduce((sum, f) => sum + getContributionValue(f, 'N', 'n_pct', f.dose_kg_ha || f.total_dose || 0), 0) + acidContribTotals.N,
      NH4: fertSource.reduce((sum, f) => sum + getContributionValue(f, 'NH4', 'nh4_pct', f.dose_kg_ha || f.total_dose || 0), 0),
      P2O5: fertSource.reduce((sum, f) => sum + getContributionValue(f, 'P2O5', 'p2o5_pct', f.dose_kg_ha || f.total_dose || 0), 0) + acidContribTotals.P2O5,
            ...
            <div className="wizard-panel" style={{ marginBottom: '0' }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table className="wizard-table">
                  ...
                  <tbody>
                    {fertAndAcidsSource.map((f, i) => {
                      const dose = f.dose_kg_ha || f.total_dose || 0;
                      const isAcid = f.isAcid || false;

                      const nContrib = isAcid ? (f.contributions?.N || 0) : getContributionValue(f, 'N', 'n_pct', dose);
                      const nh4Contrib = isAcid ? 0 : getContributionValue(f, 'NH4', 'nh4_pct', dose);
                      const pContrib = isAcid ? (f.contributions?.P2O5 || 0) : getContributionValue(f, 'P2O5', 'p2o5_pct', dose);
                      const kContrib = isAcid ? 0 : getContributionValue(f, 'K2O', 'k2o_pct', dose);
                      const caContrib = isAcid ? 0 : getContributionValue(f, 'Ca', 'ca_pct', dose);
                      const mgContrib = isAcid ? 0 : getContributionValue(f, 'Mg', 'mg_pct', dose);
                      const sContrib = isAcid ? (f.contributions?.S || 0) : getContributionValue(f, 'S', 's_pct', dose);

                      return (
                        <tr key={i} style={{ background: isAcid ? '#fffbeb' : 'transparent' }}>
                          <td className="font-bold" style={{ color: isAcid ? '#b45309' : 'inherit' }}>
                            {f.fertilizer_name || f.name}
                          </td>
                          <td className="text-right">{dose.toFixed(1)} {isAcid ? 'L' : 'kg'}</td>
              ...
              <p style={{ fontSize: '0.75rem', color: '#1e40af', marginTop: '12px', background: '#dbeafe', padding: '8px 12px', borderRadius: '6px' }}>
                Los aportes están expresados en kg/ha para la etapa actual. Ácidos muestran dosis en L/ha.
              </p>
            </div>
          )}
        </div>
